...
**RPWM** default: **GPIO 11** (physical pin 23). With **SPI disabled** (raspi-config), GPIO 11 is free for PWM — no conflict with SPI0 SCLK.
...
RPWM_PIN = 11  # BCM — was 4; keep GPIO 4 free for DS18B20 1-Wire
...
def suction_pump_on(speed):
    """Start flask suction pump continuously (same name as before for main.py)."""
    _ensure_pwm()

    speed = max(0, min(100, float(speed)))
    print(f"Flask suction pump ON: {speed:.1f}%")

    rpwm.ChangeDutyCycle(speed)


def suction_pump_off():
    """Stop flask suction pump."""
    if _pwm_initialized:
        print("Flask suction pump OFF")
